...
check=$(lsof -i:80 | cut -d' ' -f1 | awk 'NR==2 {print $1}')
if [[ ! -z "$check" ]]; then
sleep 1
echo -e "${RB}[ WARNING ]${NC} ${YB}Detected port 80 used by $check${NC} "
systemctl stop $check
sleep 2
echo -e "${GB}[ INFO ]${NC} ${YB}Processing to stop $check${NC} "
sleep 1
fi
...
sleep 2
echo -e "${GB}[ INFO ]${NC} ${YB}Starting service $check${NC} "
sleep 2
echo "$domain" > /usr/local/etc/xray/domain
systemctl restart $check
